...
const daysBetween = (a: number, b: number) =>
  Math.max(0, Math.ceil((b - a) / 86400000));
...
function probOTM_forward(
  ...
  if (![S, K, ivFrac, Tyears].every((v) => typeof v === "number" && v > 0))
    return null;
...
function interpolateIV_logMoneyness(
  ...
  const pts = points
    .filter(
      (p) => isFinite(p.K) && p.K > 0 && isFinite(p.ivFrac) && p.ivFrac > 0
    )
...
export default function App() {
  ...
  const runFlow = async () => {
    ...
    try {
      ...
      setErr(e?.message || "Options fetch failed");
    } finally {
      setLoading(false);
    }
  };
    ...
    for (const target of DTE_BUCKETS) {
      ...
      for (const o of near.ex.options) {
        ...
        let ivFrac: number | null =
          typeof o.iv === "number" && o.iv > 0
            ? o.iv / 100
            : interpolateIV_logMoneyness(uPrice, ivPoints, o.strike);
        if (ivFrac == null || !isFinite(ivFrac) || ivFrac <= 0) continue;

        const p = probOTM_forward(
          o.type,
          uPrice,
          o.strike,
          ivFrac,
      ...
      {view === "yields" && topPuts && uPrice != null && (
        <div className="yields-panel">
          <div className="y-meta">
            Data timestamp:{" "}
            <strong>{dataTimestamp ?? new Date().toLocaleString()}</strong>
          </div>
